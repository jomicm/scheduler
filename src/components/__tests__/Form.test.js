...
describe("Form", () => {
  ...
  xit("calls onSave function when the name is defined", () => {
    ...
    const onSave = jest.fn();
    /* 2. Render the Form with interviewers, name and the onSave mock function passed as an onSave prop */
    const { queryByText, getByText } = render(<Form interviewers={interviewers} name='Lydia Miller-Jones' interviewer={1} onSave={onSave}/>)
    /* 3. Click the save button */
    fireEvent.click(getByText('Save'));
    /* 4. Expectations */
  ...
  xit("submits the name entered by the user", () => {
    const onSave = jest.fn();
    const { getByText, getByPlaceholderText } = render(<Form interviewers={interviewers} interviewer={1} onSave={onSave} />);
